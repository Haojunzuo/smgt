...
import com.wbz.system.domain.vo.ConstantInfo;
import com.wbz.system.mapper.*;
import com.wbz.system.service.CourseService;
import com.wbz.system.utils.ClassSchedulUtil;
...
public class CourseServiceImpl implements CourseService {
    ...
    @Autowired
    public ClassTaskMapper classTaskMapper;
    @Autowired
    LocationInfoMapper locationInfoMapper;
    @Autowired
    ClassroomLocationMapper classroomLocationMapper;
    @Autowired
    ...
    @Override
    public int clearCoursePlan() {
        return coursePlanMapper.clearCoursePlan();
    }
    ...
    public List<String> finalResult(Map<String, List<String>> individualMap) {
        ...
        List<String> collegeNoList = classTaskMapper.selectByColumnName(ConstantInfo.COLLEGE_NO);//学院编号集合
        Map<String,List<String>> map = geneByCollege(resultGeneList,collegeNoList);//将基因按学院分配
        for (String collegeNo : map.keySet()) {
            String teachBuildNo = locationInfoMapper.selectBuildNo(collegeNo);//根据教务处划分的教学区域，查询学院对应的教学楼编号
            List<String> tempResultGeneList = map.get(collegeNo);//根据学院编号查询出该学院下的基因准备开始安排教室
            //根据教学楼编号查询出该教学楼下所有的教室来进行随机分配
            List<ClassroomLocation> classroomLocationList = classroomLocationMapper.selectByTeachBuildNo(teachBuildNo);
    ...
    public String assignClassroom(String gene, List<ClassroomLocation> classroomLocationList, List<String> resultList) {
        List<ClassroomLocation> sportsBuildingInfo = classroomLocationMapper.selectByTeachBuildNo("08");//体育大楼
        List<ClassroomLocation> experimentalBuildingInfo = classroomLocationMapper.selectByTeachBuildNo("09");//实验大楼
        List<ClassroomLocation> medicalBuildingInfo = classroomLocationMapper.selectByTeachBuildNo("10");//医学实验大楼

        String classNo = ClassSchedulUtil.cutGene(ConstantInfo.CLASS_NO, gene);
        int studentNumber = classInfoMapper.selectStudentNumber(classNo);
